...
const NFTDetails = () => {
	...
	const { endAuction, placeBid, createAuction, remainingTime } = useManageAuctions({
	...
	return (
		<NFTDetailsContainer>
			<MainNFTAndButtonsContainer>
				...
				<ButtonsContainer>
					{!localStorage.getItem(`${nftData.token_address}/${nftData.token_id}`)?.includes("0x") ? (
                      ...
                      {lastBids &&
						...
						<SmartContractWalletAddress>
							<Link
								to={"https://sepolia.etherscan.io/address/" + { createdAccount }}
								target="_blank"
				...
				{parseFloat(balance?.formatted) < parseFloat(bidValue) && (
					<InfoMessageText>
						Your balance {parseFloat(balance?.formatted).toFixed(2)} cannot bid {parseFloat(bidValue)} ETH
						on this NFT.
					</InfoMessageText>
				)}
			...
			<NftsOfMainNftContainer>
				...
				{remainingTime !== undefined && (
					<Countdown date={Date.now() + Number(remainingTime) * 1000} renderer={renderer} />
				)}
				...
				{Number(remainingTime) == 0 && <CountdownInfoText>The auction has ended.</CountdownInfoText>}
